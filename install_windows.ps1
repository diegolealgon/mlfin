...
function Die  {
    ...
    Write-Host ""
    Write-Host "   A detailed log was saved to:"
    Write-Host "   $LogFile"
    Write-Host ""
...
function Invoke-Tool {
    <#
        Runs a command line program, sends everything it prints to the log,
        and decides success from the exit code.

        This wrapper exists because of a PowerShell trap. With
        $ErrorActionPreference set to "Stop", PowerShell turns ANY line a
        native program writes to stderr into a terminating error. Conda and
        pip both write ordinary warnings to stderr, so a harmless notice
        would abort the install at the moment it was actually succeeding.
        Dropping to "Continue" for the duration of the call avoids that, and
        the exit code tells us whether it really failed.
    #>
    param(
    ...
    $code = 1
    try {
        Add-Content -Path $LogFile -Value "`r`n--- $Exe $($Arguments -join ' ') ---"
        & $Exe @Arguments 2>&1 | Out-File -FilePath $LogFile -Append -Encoding utf8
        $code = $LASTEXITCODE
    } catch {
    ...
    if ($code -ne 0 -and $FailMessage) { Die $FailMessage }
    return $code
}
...
Invoke-Tool -Exe $Conda -Arguments @("create", "-y", "-q", "-n", $EnvName, "python=$PythonVersion") | Out-Null
if (-not (Test-Path $Py)) {
...
Invoke-Tool -Exe $Pip `
    -Arguments @("install", "--no-input", "-q", "-r", (Join-Path $assets "requirements.txt")) `
    -FailMessage "Package installation failed. Please send the log file named below to whoever sent you this installer." | Out-Null

# Clear kernel name, so the picker is unambiguous if another Python exists.
Invoke-Tool -Exe $Py `
    -Arguments @("-m", "ipykernel", "install", "--user", "--name", $EnvName,
                 "--display-name", "ML_Finance (Python $PythonVersion)") | Out-Null
